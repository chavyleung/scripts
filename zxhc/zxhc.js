...
// 若智行更换 soa2 版本号，请将 18903 改为新版本号
const signURL = 'https://m.suanya.cn/restapi/soa2/18903/attendanceDay'
...
function getCreditInfo() {
  return new Promise((resolve) => {
    const creditHeaderVal = $.getdata(signheaderKey)
    const creditBodyVal = $.getdata(signbodyKey) || ''
    if (!creditHeaderVal || !creditBodyVal) {
      return resolve({ credit: null, coupon: null })
    }
    const opts = {
      url: creditURL,
      headers: $.toObj(creditHeaderVal, {}),
      body: creditBodyVal,
    }
    $.post(opts, (err, resp, data) => {
      try {
